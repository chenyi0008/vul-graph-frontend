...
export const getCveList = (params: { min: number, max: number, type: string, id: string }): Promise<CveResponse> => {
  console.log('请求参数类型检查:', params)
  return request.get<CveResponse>('/cve', 
    { params: { min: params.min, max: params.max, type: params.type, id: params.id } }
  )
}
